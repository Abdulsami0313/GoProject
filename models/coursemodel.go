...
func (m *CourseModel) FindAll(course *[]entities.Course) error {
	...
	for rows.Next() {
		...
		rows.Scan(&data.Id, &data.CourseName, &data.Lesson, &data.Week, &data.Price, &data.Image, &data.Description)
...
func (m *CourseModel) Create(course *entities.Course) error {
	...
	result, err := m.db.Exec("insert into course (coursename,lesson,week,price,image,description)values(?,?,?,?,?,?)", course.CourseName, course.Lesson, course.Week, course.Price, course.Image, course.Description)
	if err != nil {
...
func (m *CourseModel) Update(course entities.Course) error {
	...
	_, err := m.db.Exec("update course set coursename = ?, lesson = ?, week = ?, price = ?, image = ? , description = ? where id = ?", course.CourseName, course.Lesson, course.Week, course.Price, course.Image, course.Description, course.Id)
